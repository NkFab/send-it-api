...
export default class UserCont {
  static signUp(req, res) {
    ...
    const query = {
      text:
        "INSERT INTO users (username, pass, email, user_type, created_on) VALUES ($1,$2,$3,$4,$5 ) RETURNING *",
  ...
  static signIn(req, res) {
    ...
    pool(query)
      .then(user => {
        ...
        bcrypt
          .compare(pass, user.rows[0].pass)
          .then(matched => {
            if (matched) {
              let payload = { ...user.rows[0] };
              const { username, user_type } = user.rows[0];
              delete payload.pass;
              jwt.sign(
                payload,
                jwtsecret.secret,
